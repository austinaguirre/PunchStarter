...
exports.lambdaHandler = async (event, context) => {
    ...
    let updateDesgnerFunds = (Designer_ID, theAddValue) => {
        return new Promise((resolve, reject) => {
    ...
    let insertTransaction = (theAddValue, Supporter_ID, Project_ID) => {
        return new Promise((resolve, reject) => {
            pool.query("insert into Transaction (Transaction_ID, Amount, Supporter_ID, Project_ID, Pledge_ID) values(uuid(),?,?,?,?);", [theAddValue,Supporter_ID,Project_ID,null], (error, result) => {
                if (error) { return reject(error); }
                if (result.affectedRows == 1) {
                    return resolve(true); // TRUE correct user and password
                }
    ...
    try {
        ...
        if (!isProject) {
            ...
            updateDesgnerFunds(Designer_ID, theAddValue);
            updateProjectCurrentAmount(Project_ID, theAddValue);

            const insertedTransaction = await insertTransaction(theAddValue, Supporter_ID, Project_ID);
